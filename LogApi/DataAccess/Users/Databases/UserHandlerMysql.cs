...
{
    public class UserHandlerMysql : IUserHandler
    {
        private readonly string _connectionString;

        public UserHandlerMysql(string connectionString)
        {
        ...
        public void AddUser(LoggedInUser user)
        {
            using (var connection = new MySqlConnection(_connectionString))
            {
                ...
                using (var checkCommand = new MySqlCommand("SELECT ID FROM LoggedInusers WHERE userPrincipalName = @UserPrincipalName", connection))
                {
                    ...
                    if (existingUserId == null)
                    {
                        ...
                        using (var insertUserCommand = new MySqlCommand("INSERT INTO LoggedInusers (displayName, jobTitle, userPrincipalName, MobileNo, Timestamp) VALUES (@DisplayName, @JobTitle, @UserPrincipalName, @MobileNo, @Timestamp); SELECT LAST_INSERT_ID();", connection))
                        {
                    ...
                    else
                    {
                        ...
                        using (var updateUserCommand = new MySqlCommand("UPDATE LoggedInusers SET Timestamp = @Timestamp WHERE ID = @ID", connection))
                        {
        ...
        public List<LoggedInUser> GetAllUsers()
        {
            ...
            using (var connection = new MySqlConnection(_connectionString))
            {
                ...
                using (var command = new MySqlCommand("SELECT ID, displayName, jobTitle, userPrincipalName, MobileNo, Timestamp FROM LoggedInusers", connection))
        ...
        public void DeleteUser(int id)
        {
            using (var connection = new MySqlConnection(_connectionString))
            {
                ...
                using (var checkCommand = new MySqlCommand("SELECT ID FROM LoggedinUsers WHERE ID = @ID", connection))
                {
                    ...
                    if (existingUserId != null)
                    {
                        ...
                        using (var deleteCommand = new MySqlCommand("DELETE FROM LoggedinUsers WHERE ID = @ID", connection))
                        {
